...
0. PROJECT BOOTSTRAPが共通SAHOU authorityを参照する場合、SAHOU current refのexact SHAを確認する。shared exact-SHA repository snapshotのintegrityを確認し、`specs/README_共通仕様セット.md` をrouting indexとして、GitHub運用specとtaskに必要なmodule / dependency closureだけをconversation contextへloadする。cache snapshot全体をcontextへ展開しない。SHAが変わっている場合のみshared cacheを再取得・更新し、派生 `SAHOU_FULL.md` 等は作らない。
...
共通SAHOUを利用するprojectでは、少なくともSAHOU repository identity、target ref、`load=routed`、routing index、shared exact-SHA snapshot cacheを利用することが分かる参照を持たせる。projectごとにSAHOU全文またはSAHOU cacheを複製せず、taskに必要なmoduleだけをloadする。
